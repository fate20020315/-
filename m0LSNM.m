...
%eta=1/k;
  ...
  d= hession(g,h,root(k,:),eta) ;
    s(k)=armijos(@(x)norm_g(g,x), root(k,:),k, rho, d') ;  
  else
 d= minihession(g,h,root(k,:),i,eta);
  sns=sns+ceil(0.05*ns);
  i=unidrnd(ns,sns,1,1); 
   s(k)=armijos(@(x)norm_g1(g,x,i), root(k,:),k, rho, d') ;
  end
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%产生方向  
...
time(k) = toc(iter_start);  % 记录这一迭代的时间
    if k > 1
        time(k) = time(k) + time(k - 1);  % 累计时间
    end 
end
toc
for j=1:iter+1
...
plot(q);
final_solution = root(end,:); % 获取最后一次迭代的解
    disp('最终的解为：');
    disp(final_solution);
